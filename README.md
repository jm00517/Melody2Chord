...
현재는 규칙 기반 `v1` 생성기이며, CLI와 다중 후보 비교용 Web UI를 함께 제공합니다.
...
- Web UI에서 한 번에 여러 진행 샘플을 뽑고 후보별 비교, 전체 리롤, 코드만 리롤이 가능합니다.
...
- ? ? ???? ???? ?? ?????.
- ???? MIDI ??? `.uploads` ??? ??? ? ??? ?????.
- ??? ??? ?? ??? ????, ? ???? progression/drum/bass ??? ?? ??? ??? ? ????.
- `Reroll All`? ?? ??? ?? ??, `Reroll Chords`? ???? ??? ??? ? ??/??? ??? ?? ?????.
...
- 여러 후보를 카드로 비교하고 브라우저 안에서 하나를 선택 표시할 수 있습니다.
...
## 16. 향후 확장 포인트
- Web UI는 빠른 실험용이며 파일 매니저나 오디오 미리듣기를 제공하지 않습니다.
- 멜로디 phrase 기반 섹션 분리
- `Select This`는 브라우저 내부 선택 표시이며, FL 프로젝트에 자동 반영되지는 않습니다.
- 코드 텐션, 전위, 보이싱 개선
- 베이스 리듬 다양화
- 장르별 드럼 템플릿 고도화
